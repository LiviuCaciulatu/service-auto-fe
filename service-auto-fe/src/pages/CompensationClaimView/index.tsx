...
export default function CompensationClaimView() {
  const { id: clientId, claimId } = useParams<{ id: string; claimId: string }>()
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    // Use relative path so Vite dev server proxy forwards request to backend and avoids CORS
    fetch(`/compensationClaims/${claimId}`)
      .then(async res => {
        ...
        if (res.ok) {
          try {
            ...
            setItem(parsed)
            // Prefill edit form and open it immediately
  ...
  return (
    <main className="home-container">
      ...
      <h1 className="home-header">Compensation Claim</h1>
            ...
            {Object.entries(form).map(([k, v]) => (
              <div className="form-row" key={k}>
                ...
                {typeof v === 'string' || typeof v === 'number' || typeof v === 'boolean' ? (
                  ...
                  <input className="form-input" value={(v as any).map((it:any)=>typeof it==='object'?JSON.stringify(it):String(it)).join(', ')} onChange={e => setForm((s:any)=>({...s, [k]: e.target.value.split(',').map((p:string)=>p.trim()).filter(Boolean)}))} />
                ) : (
